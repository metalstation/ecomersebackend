    ...
    async (req, res) => {
        ...
        try {
            ...
            if (VerificationUser.otp === req.body.otp) {
                ...
                const authToken = jwt.sign(data, JWT_SECRET);
                res.status(200).json({ success: true, authToken: authToken, user: newuser });
            }
            else {
...
router.post('/login',
    ...
    async (req, res) => {
        ...
        try {
            ...
            const authToken = jwt.sign(data, JWT_SECRET)
            success = true
            res.status(200).json({ success, authToken, user: user });
    ...
    async (req, res) => {
        try {
            ...
            if (address) {
                ...
                user.address.location = address.location;
            }
            if (phone) {
                user.phone = phone;
            }
